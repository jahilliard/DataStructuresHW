...
public class ColumnPuzzle implements MouseListener {
	...
	public boolean isSolved() {
		for (int col=0; col<getNumCols(); col++) {
			for (int row=1; row<getNumRows(); row++){
				if ((grid[row][col]!=(Color.BLACK)) && (grid[row][col]!=(Color.MAGENTA)) &&
						(grid[row-1][col]!=(Color.BLACK)) && (grid[row-1][col]!=(Color.MAGENTA))){
	...
	public void mousePressed(MouseEvent e) {
		...
		if(!isSolved()) {
			...
			int[] arr = closestBlackSquare(row,col);
